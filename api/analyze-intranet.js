...
// Added: Series-aware Official Rule mapping
// January 09, 2026
...
// NEW: Series-aware Official Rule mapping (Option #2)
const ruleMap = {
  "Formula 1 (F1 2xxx / Codemasters)": "FIA Formula 1 Sporting Regulations – Articles 27–29 (Overtaking, Track Limits, and Driver Conduct)",
  "NASCAR / Oval Racing": "NASCAR Rule Book – Section 10.5: On-Track Conduct & Contact Guidelines",
  "iRacing": "iRacing Official Sporting Code – Section 8.1: Racing Conduct and Situational Awareness",
  "Assetto Corsa Competizione (ACC / LFM)": "LFM Sporting Regulations – Article 8: Overtaking and Defensive Moves",
  "Gran Turismo 7": "Gran Turismo Sport & GT7 Sporting Code – Etiquette & Penalty Guidelines",
  "Forza Motorsport": "Forza Motorsport Sporting Code – General Racing Conduct",
  "rFactor 2": "rFactor 2 League & General Racing Rules",
  "Dirt / Rallycross": "Rallycross & Dirt Racing Etiquette Guidelines",
  "BeamNG.drive": "BeamNG.drive Community Racing Guidelines",
  "Other / League-Specific Rules": "Relevant League or Series Sporting Code",
  // Default fallback if series not matched
  "": "Relevant Sim Racing Sporting Code / Etiquette Guidelines"
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Load curated precedents with series-aware filtering
    let precedentCases = [];
...
Return ONLY valid JSON:
{
  ...
  "confidence": "${confidence}"
    ...
    // NEW: Use series-aware rule from mapping (fallback to generic if no match)
    const selectedRule = ruleMap[series.trim()] || "Relevant Sim Racing Sporting Code / Etiquette Guidelines";

    let verdict = {
      rule: selectedRule,  // ← Now dynamic based on series
      fault: { [`Car A${carAIdentifier}`]: `${finalFaultA}%`, [`Car B${carBIdentifier}`]: `${100-finalFaultA}%` },
      car_identification: carIdentification,
