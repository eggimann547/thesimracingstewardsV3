...
export default function Home() {
  ...
  const [stewardNotes, setStewardNotes] = useState('');        // ← NEW: long human notes
  const [overrideFaultA, setOverrideFaultA] = useState('');    // ← NEW: optional override
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only send override if the user actually typed something
      ...
      <div className="max-w-6xl mx-auto p-6 md:p-12 space-y-10">
        <div className="text-center">
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
            Sim Racing Steward AI — Human-in-the-Loop
          </h1>
          <p className="text-lg text-gray-600 dark:text-gray-300 mt-2">
            Real stewards + 28k precedents + Grok-3 → perfect verdicts every time
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
          ...
          {/* NEW: Steward Notes – replaces weak description */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Steward Notes <span className="text-gray-500">(what you actually saw – drives the verdict)</span>
            </label>
            <textarea
              value={stewardNotes} onChange={(e) => setStewardNotes(e.target.value)} rows={5}
              placeholder="e.g. Car A braked 18 m late, had <30% overlap at apex, ignored inside line commitment..."
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Override Fault % on Car A <span className="text-gray-500">(optional – leave blank to use precedent average)</span>
            </label>
            <input
              type="number" min="0" max="100" value={overrideFaultA} onChange={(e) => setOverrideFaultA(e.target.value)}
              placeholder="e.g. 92"
            ...
            {result && (
              <div className="p-8 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-2xl border border-blue-200 dark:border-blue-700">
                ...
                <div className="space-y-6 text-gray-800 dark:text-gray-200">
                  <div><strong className="text-xl text-blue-600 dark:text-blue-400 block mb-2">Rule</strong><p className="text-lg">{result.rule}</p></div>
                  <div><strong className="text-xl text-blue-600 dark:text-blue-400 block mb-2">Car Roles</strong><p className="text-base italic p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl font-semibold">{result.car_identification}</p></div>
                  <div>
                    <strong className="text-xl text-blue-600 dark:text-blue-400 block mb-2">Confidence</strong>
                    <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-bold ${result.confidence.includes('Override') || result.confidence.includes('High') ? 'bg-green-100 text-green-800' : result.confidence === 'Medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                      {result.confidence} {result.confidence === 'Human Override' ? '' : `(${matches.length}/5 matches)`}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 p-5 bg-gradient-to-r from-red-50 to-green-50 dark:from-red-900/20 dark:to-green-900/20 rounded-xl">
                    <div className="text-center">
                      ...
                      <span className="text-4xl font-bold block">{Object.entries(result.fault).find(([k]) => k.includes('Car A'))?.[1] || '—'}</span>
                    </div>
                    <div className="text-center">
                      <strong className="text-lg text-green-600 dark:text-green-400">Car B {carB && `(${carB})`}</strong>
                      <span className="text-4xl font-bold block">{Object.entries(result.fault).find(([k]) => k.includes('Car B'))?.[1] || '—'}</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-inner">
                    ...
                    <p className="mt-6 italic text-lg font-semibold text-blue-700 dark:text-blue-300">Pro Tip: {result.pro_tip}</p>
                  </div>

                  <div className="flex gap-3">
                    <button onClick={() => navigator.clipboard.writeText(`[VERDICT]\n${result.car_identification}\nFault: A ${Object.entries(result.fault).find(([k]) => k.includes('Car A'))?.[1]} / B ${Object.entries(result.fault).find(([k]) => k.includes('Car B'))?.[1]}\n${result.explanation}\n\nTip: ${result.pro_tip}`) || alert('Copied!')}
                      className="flex-1 p-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold">Copy Full Verdict</button>
                    <button onClick={() => navigator.clipboard.writeText(`A ${Object.entries(result.fault).find(([k]) => k.includes('Car A'))?.[1]} / B ${Object.entries(result.fault).find(([k]) => k.includes('Car B'))?.[1]}`) || alert('Copied!')}
                      className="flex-1 p-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold">Copy Short</button>
                  </div>
                </div>
              </div>
            )}
