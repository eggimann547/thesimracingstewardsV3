...
export default function Home() {
  ...
  const [manualTitle, setManualTitle] = useState('');  // ← NEW: Optional manual title for Reddit/keywords
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        manualTitle: manualTitle.trim()  // ← NEW: Send to backend
      };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={carA} onChange={(e) => setCarA(e.target.value)}
            ...
            <div>
              ...
              <input type="text" value={carB} onChange={(e) => setCarB(e.target.value)}
          ...
          <div>
            ...
            <textarea value={stewardNotes} onChange={(e) => setStewardNotes(e.target.value)} rows={5}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Manual Title (optional — e.g., from Reddit post)
            </label>
            <input
              type="text" value={manualTitle} onChange={(e) => setManualTitle(e.target.value)}
              placeholder="e.g. 'Divebomb or fair overtake? Monza T1 incident'"
              className="w-full p-5 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:ring-4 focus:ring-blue-500 text-lg"
            />
          </div>
          ...
          <div>
            ...
            <input type="number" min="0" max="100" value={overrideFaultA} onChange={(e) => setOverrideFaultA(e.target.value)}
              placeholder="e.g. 92"
            ...
            {result && (
              <div className="p-8 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-2xl border border-blue-200 dark:border-blue-700">
                <h2 className="text-2xl font-bold text-blue-700 dark:text-blue-300 mb-6">
                  Official Verdict — The Sim Racing Stewards AI Tool
                </h2>
